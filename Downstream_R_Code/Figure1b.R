...
pdf("to_AI/Figure1b.pdf", width=7, height=2.2, pointsize=1, useDingbats=FALSE)
par(mfcol=c(1,4))
for (i in 1:length(selected.HPV)){
	...
	plot(percent.human, subtype.freq,
		...
		xlab = "", ylab="")
	mtext("Percent Human",1, cex=1, padj=3, font=2)
	mtext(paste("Percent ",plot.type," Reads",sep=""),2, cex=1, padj=-4, font=2)
