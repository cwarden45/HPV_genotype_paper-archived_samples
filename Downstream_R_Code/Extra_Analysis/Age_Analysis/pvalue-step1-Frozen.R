...
meta.table = meta.table[!is.na(meta.table$Age)&(meta.table$batch == "Frozen"),]
print(dim(meta.table))
...
##with 15% rather than 5% read fraction, there are no Frozen tissue co-infections
...
FE.group[!is.na(meta.table$Age) & (meta.table$Age >= 50)]="gt50"
FE.group[!is.na(meta.table$Age) & (meta.table$Age < 50)]="lt50"
...
detection.trt = grp.percent.detected$gt50
detection.ref = grp.percent.detected$lt50
...
fe.status[(fe.fdr < fdr.cutoff)&(detection.diff > 10)]=">50 Up"
fe.status[(fe.fdr < fdr.cutoff)&(detection.diff < -10)]="<50 Down"
...
avg.date = data.frame(t(apply(status.mat, 2, avg.date.by.status, dates=meta.table$Age)))
...
ks.pvalue = apply(status.mat, 2, ks.date, meta.table$Age)
ks.fdr = p.adjust(ks.pvalue, "fdr")
ks.status = rep("No Change",length(ks.fdr))
ks.status[(ks.fdr < fdr.cutoff)&(avg.diff > 0)]=">50 Up"
ks.status[(ks.fdr < fdr.cutoff)&(avg.diff < 0)]="<50 Down"
...
bGLM.pvalue = apply(status.mat, 2, calc.bGLM.pvalue, var1=meta.table$Age)
...
lm.pvalue = apply(ab.mat, 1, calc.lm.pvalue, var1=meta.table$Age, var2=percent.human)
