...
pdf("HPV_genotype_by_Collection_Date.pdf", width=30, height=10, useDingbats=FALSE)
par(mfcol=c(1,3))
for (i in 1:length(selected.HPV)){
	...
	par(mar=c(15,10,7,7))
	plot(meta.table$collection.year,subtype.freq,cex.main=3, cex.axis=3,cex.lab=3,
	...
	text(1985,112, labels=plot.type, xpd=T, cex=3, font=2)
	mtext("Collection Year",1, cex=2, padj=6)
	mtext(paste("Percent ",plot.type," Reads",sep=""),2, cex=2, padj=-4)
	legend(2005,115, legend = c("Frozen","FFPE"), col="black", pt.bg =c("orange","cyan"),
