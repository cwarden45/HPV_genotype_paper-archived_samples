...
grp.percent.detected = round(100*data.frame(t(apply(status.mat, 2, percent.detected.by.group, group=meta.table$ADMIXTURE.mixed))), digits=1)
detection.trt = grp.percent.detected$AMR 
detection.ref = grp.percent.detected$EUR
detection.diff = detection.trt - detection.ref

fe.pvalue = apply(status.mat, 2, calc.fe.pvalue, grp=meta.table$ADMIXTURE.mixed)
