sample_type = "Frozen"
compID = "Frozen-SCC_vs_AdenoTypes-1var-qPCR20"
...
calc.fe.pvalue = function(binary.arr, grp){
	grp_levels = table(grp)
	status_levels = table(binary.arr)
	if((length(grp_levels) == 1)|(length(status_levels) == 1)){
		return(NA)
	}else{
...
meta.table$hist.subtype = factor(meta.table$hist.subtype,levels = c("AdenoTypes","SCC"))
print(dim(meta.table))
